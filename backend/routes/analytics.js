...
router.get('/analytics/recent-food', async (req, res) => {
    try {
...
router.get('/analytics/sensor-activity', async (req, res) => {
    try {
...
router.get('/analytics/food-risk', async (req, res) => {
    try {
        const { date, start, end } = req.query;
        let query = 'SELECT name, expiration_date FROM food_items';
        let params = [];
        if (date) {
            ...
            query += ' WHERE expiration_date BETWEEN ? AND ?';
            params.push(start, end);
        }
        const [foods] = await db.query(query, params);
        const today = new Date();
        ...
        foods.forEach(food => {
            const expDate = new Date(food.expiration_date);
            if (expDate < today || (expDate - today) / (1000*60*60*24) <= 3) {
                risky++;
            }
        });
        const total = foods.length;
        const score = total > 0 ? Math.round((risky / total) * 100) : 0;
        res.json({ success: true, score, total, risky });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});
